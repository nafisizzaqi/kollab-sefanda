        ...
        <tbody>
            @forelse ($gurus as $guru)
                <tr>
                    ...
                    <td>
                        @if ($guru->image)
                            <img src="{{ asset('storage/' . $guru->image) }}" alt="Gambar" width="100">
                        @else
                            -
                        @endif
                    </td>
                    <td>
                        ...
                        <form action="{{ route('pages.guru.destroy', $guru->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus guru ini?')">
                            @csrf
                            @method('DELETE')
